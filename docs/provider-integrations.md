...
The real synthetic-audio WebRTC check caught an onset hallucination when the voice model answered without delegation. Live instructions now carry only student-visible, immediately available authored history (not physical-exam findings, future facts, or hidden scoring content) and require bedside/action requests to await application results. Reverification recognized synthetic onset/preparation questions, spoke the correct 45-minute onset, emitted a client delegation, and spoke the server-confirmed draft-only response. The receipt at `docs/evidence/phase-05/real-live-synthetic-check.json` is a real provider check, not human microphone validation.
