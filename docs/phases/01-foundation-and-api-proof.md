...
- [x] V04: Run the browser probe: microphone input produces a real spoken response and interruption works.
...
- Commands / manual steps: `npm ci`; `npm run verify`; `npm start` with local HTTP checks; `npm run probe:examiner`. See [setup and verification evidence](../evidence/setup-and-verification.md). On 13 September 2026, the user independently ran the browser probe with microphone access, heard a real spoken response, and verified interruption.
- Results / artifact locations: automated checks passed (2 files / 10 tests; 3 client build files scanned); real Astra managed session and application function round trip passed; the human-observed Live audio gate passed. Sanitized receipt: [examiner-session-receipt.json](../evidence/examiner-session-receipt.json). Clinical review: [clinical-review-checklist.md](../clinical-review-checklist.md).
- Remaining blockers: None. Clinical content intentionally remains `draft_unreviewed`; that status does not block this API-proof phase but must not be presented as educationally validated.
